...
另外，需要注意的是：**这两种方式不能交换使用**，get值和set值必须使用同一种方法。
...
**补充**：上面的规律总结，适用于大部分标签属性（比如 普通标签的`class/className`属性、img 标签的`src`属性、超链接的`href`属性等），但 `title`属性例外。

实验表明：`title`属性，无论是方式一、还是方式二，都是有效的，且可以交换使用。这一点，比较特殊。如果还有其他属性也例外，欢迎提交 issues 补充。
